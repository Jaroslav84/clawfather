...
- **Mitigation**: Always audit skills using `skill-scanner` and only download from trusted sources like [ClawHub.ai](https://clawhub.ai).
...
| Feature | Local Machine Installation | Docker / Containerized |
| :--- | :--- | :--- |
| **Isolation** | **None**. Agent has access to your `$HOME` and system. | **High**. Filesystem is restricted to mapped volumes. |
| **Hardware Access** | **Direct**. Full access to GPU, Camera, Mic, UI. | **Restricted**. Requires complex pass-through for UI/Hardware. |
| **macOS Integration** | **Deep**. Supports AppleScript, TCC, menu bar. | **Limited**. Hard to trigger native macOS UI/TCC. |
| **Persistent Data** | Lives in your standard app data folders. | Managed via Docker Volumes. |
| **Security Risk** | **High**. Escape to host is trivial (it's already there). | **Medium**. Escape is much harder, but not impossible. |
