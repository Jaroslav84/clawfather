...
EOF
    ...
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "TUI (chat):" "$RESET" "docker compose exec -it $GATEWAY_SERVICE node dist/index.js tui"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Help:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js --help"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Gateway stop:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js gateway stop"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Audit Deep:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js security audit --deep"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Audit Fix:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js security audit --fix"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Sync Skills:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js skill sync"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Scan Models:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js model scan"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Environment:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js env"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Status:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js status --all"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Version:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js --version"
    printf "%b│%b\n" "$accent_color" "$RESET"
    printf "%b│%b    %b[ Gateway Config ]%b\n" "$accent_color" "$RESET" "$BOLD" "$RESET"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Config get:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js config get <key>"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Config set:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js config set <key> <value>"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Config unset:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js config unset <key>"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "config.get:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js gateway call config.get --params '{}'"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "config.apply:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js gateway call config.apply --params '<raw> <baseHash>'"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "config.patch:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js gateway call config.patch --params '<raw> <baseHash>'"
    printf "%b│%b      %b%-18s%b %s\n" "$accent_color" "$RESET" "${dim_color:-$DIM}" "Doctor:" "$RESET" "docker compose exec $GATEWAY_SERVICE node dist/index.js doctor"
    printf "%b│%b\n" "$accent_color" "$RESET"
